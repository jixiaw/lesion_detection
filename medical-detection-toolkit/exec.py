...
def train(logger):
    ...
    for epoch in range(starting_epoch, cf.num_epochs + 1):
        ...
        seen_pids = []
        # for bix in range(5):
        for bix in range(batch_gen.train_steps):
            batch = batch_gen.next_train_batch()
            # print(batch)
            ...
            optimizer.step()
            if bix % 10 == 0:
                logger.info('\rtr. batch {0}/{1} (ep. {2}) fw {3:.2f}s / bw {4:.2f} s / total {5:.2f} s || '.format(
                bix + 1, batch_gen.train_steps, epoch, tic_bw - tic_fw, time.time() - tic_bw,
        ...
        with torch.no_grad():
            net.eval()
            if cf.do_validation:
                start_vaild_time = time.time()
                val_results_list = []
                val_predictor = Predictor(cf, net, logger, mode='val')
                for step in range(batch_gen.test_steps):
                    batch = batch_gen.next_test_batch()
                ...
                logger.info('\rtr. valid: (ep. {}) total {:.2f} s|| '.format(
                    epoch, time.time() - start_vaild_time) + results_dict['logger_string'])
