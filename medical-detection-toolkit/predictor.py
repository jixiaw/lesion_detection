...
class Predictor:
    ...
    def predict_test_set(self, batch_gen, return_results=True):
        ...
        for rank_ix, weight_path in enumerate(weight_paths):
            print("name_dict size: ", len(name_dict))
            ...
            plot_batches = np.random.choice(np.arange(batch_gen.test_steps), size=n_test_plots, replace=False)
            # name_dict = {}
            with torch.no_grad():
                for i in range(len(batch_gen.test_list)):

                    batch = batch_gen.next_patient()
                    # if batch['pid'] in name_dict:
                    #     print("ERROR, all ready exits: ", i, batch['pid'])
                    name_dict[batch['pid']] = 1
        ...
        for pid, p_dict in dict_of_patient_results.items():
            ...
            results_dict['boxes_gt'] = p_dict['patient_bb_target']
    ...
    def data_aug_forward(self, batch):
        ...
        results_dict['seg_preds'] = np.array([[item for d in results_list for item in d['seg_preds'][batch_instance]]
                                              for batch_instance in range(org_img_shape[0])])
        # results_dict['boxes'] = results_list[0]
